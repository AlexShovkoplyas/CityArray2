...
namespace MVCIdentityEmpty.Controllers
{
    [Authorize]
    public class ManageController : Controller
    {
        ...
        public ManageController(IRepo repo)
        {
            db = repo;
        ...
        public AppSignInManager SignInManager
        {
            get
            {
                return SignInManager1 ?? HttpContext.GetOwinContext().Get<AppSignInManager>();
            }
            private set 
            { 
                SignInManager1 = value; 
            }
        }
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var reviews = db.Reviews.GetAll().Where(p => p.PersonId == userId).ToList();
            var model = new IndexViewModel
            {
                HasPassword = HasPassword(),
                Logins = await UserManager.GetLoginsAsync(userId),
                Reviews = Mapper.Map<List<ReviewInfo>>(reviews)
            };
            return View(model);
        }
...
#region Helpers
        ...
        public AppSignInManager SignInManager1 { get => _signInManager; set => _signInManager = value; }
